...
def delta_min(mu):
    ...
    return np.sqrt(-4.5 * mu) + 0.00005        # small margin above boundary

def build_train_pairsA(cfg):
    """
    Increasing number of delta samples per mu:
    mu_i  →  N_delta = 2*(i+1)
    """
    mu_list   = np.linspace(cfg["mu_left"], cfg["mu_right"], cfg["N_mu"])
    n_deltas  = [5,  5, 6, 6, 7, 7,  8, 8, 9, 10, 11, 12, 13, 14, 15, 16] #
    pairs = []
    for mu, n_d in zip(mu_list, n_deltas):
        for delta in np.linspace(delta_min(mu), 1.04, n_d):
...
def build_train_pairs(cfg):
    ...
    for mu in mu_list:
        d_lo = delta_min(mu)
        n_d  = max(5, round((1.1 - d_lo) / step) + 1)
        for delta in np.linspace(d_lo, 1.1, n_d):
            pairs.append([mu, delta])
    return pairs
